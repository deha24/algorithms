...
benchmark() {
    ...
    result=$(/usr/bin/time -f "%e" ${runpython} --algorithm $algorithm < $input_file 2>&1 >/dev/null)
    echo "Result $result"
    time=$(echo $result | head -1)  # Extract real time from the time command output
    echo $time
    size=$(head -n 1 $input_file)
    # Append results to CSV file
    echo "$algorithm_name,$size,$time" >> "benchmark_results/"$input_type"_"$algorithm_name.csv
    echo "------------------------"
}
...
for input_type in "${input_files[@]}"; do
    for algorithm_name in "${!algorithm_mapping[@]}"; do
        algorithm_number=${algorithm_mapping[$algorithm_name]}
        echo "Algorithm,InputSize,Time" > "benchmark_results/"$input_type"_"$algorithm_name.csv
